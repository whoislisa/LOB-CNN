...
# num_dataset = _U.generate_numerical_dataset(df1.iloc[:10000, :], record_cnt=5)  # full numerical dataset
# labels = [entry[1] for entry in num_dataset]  # 只取ret5标签
# print(f"Label distribution: \n{pd.Series(labels).value_counts(normalize=True)}")

# df_results = _U.traditional_ml_pipeline(entries=num_dataset, balance=True)
...
model = _M.BinaryCNN()
train_loader, val_loader, test_loader = _M.create_cnn_dataloaders(image_dataset, batch_size=64, target='ret5')
print("train_loader.dataset[0][0].shape:", train_loader.dataset[0][0].shape)

_M.cnn_train(model, train_loader, val_loader,lr=1e-5, epochs=5, target='ret5')
