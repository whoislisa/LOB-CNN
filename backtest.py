...
def simulate_backtest(
    ...
    for prob, ret, ts in zip(probs, true_labels, returns, timestamps):
        signal_volume = 0.0
        direction = 0
...
def predict_and_backtest(model_class, model_path, X_test, y_test, mode="classification"):
    model = model_class().to(device)
    model.load_state_dict(torch.load(model_path))
    model.eval()
    ...
    if mode == "backtest":
        ...
        # TODO: 输入不应该是预测标签，而应该会标签1的预测概率
        preds = np.array(all_preds)
        rets = simulate_backtest(preds)  # 模拟交易
        print(f"回测指标：累计收益率={rets['cumulative_return']:.4f}，夏普比率={rets['sharpe_ratio']:.4f}")
...
if __name__ == "__main__":
    ...
    save_path = "M_BinaryCNN_3_e20_b128_l0.001_D_CSI50_T_0424-1716.pt"

    print("评估 CNN...")
    week_preds = []
    week_labels = []
    week_dates = []
    for folder in data_args['test_data_folder_list']:
        print(f"评估 {folder}...")
        test_df = load_data(
            data_folder_list=[folder],
            code_list=data_args['code_list'],
            # downsample_args=downsample_args,
        )
        X_test, y_test = preprocess_data_cnn(test_df, is_train=False)
        # TODO:
        preds, labels = predict_and_backtest(model_class, save_path, X_test, y_test, mode="backtest")

        # 保存每周预测
        week_name = os.path.basename(folder)
        np.save(f"{save_path[:-3]}_preds_{week_name}.npy", preds)
        np.save(f"{save_path[:-3]}_labels_{week_name}.npy", labels)
        week_preds.append(preds)
        week_labels.append(labels)
        week_dates.append(week_name)

    # print("分周预测完成，开始整合月度评估...")
    # monthly_metrics = evaluate_monthly(save_path, week_dates)
    # monthly_metrics.to_csv(f"{save_path[:-3]}_monthly_metrics.csv", index=True)
    # print("月度评估完成。")
